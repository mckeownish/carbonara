...
./predictMulti $ScatterFile $fileLocs $initialCoordsFile $pairedPredictions $fixedsections $noStructures $withinMonomerHydroCover $betweenMonomerHydroCover $kmin $kmax $maxNoFitSteps $predictionFile/mol $scatterOut/scatter.dat $mixtureFile $prevFitStr $logLoc/fitLog.dat $endLinePrevLog $affineTrans
