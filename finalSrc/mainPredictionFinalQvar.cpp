...
int main(int argc, const char* argv[]) {
  ...
  std::vector<int> numberOfChainsInEachStructure = findNumberSections(mol);
  ...
  while (fitStep < params.noScatterFitSteps) {
    ...
    for (int structureIndex = 0; structureIndex < mol.size(); structureIndex++) {
      int netIndex = 0;
      ...
      for (int chainNumber = 1; chainNumber <= numberOfChainsInEachStructure[structureIndex]; chainNumber++) {
        ...
        if (params.affineTrans == true) {
          ...
          ktlMolecule molCopyR = mol[structureIndex];
          ...
          if (cacaDist == false) {
            ...
            if (checkTransition(newOverallFit.first, overallFit.first, uProb, fitStep, params.noScatterFitSteps)) {
              ...
              improvementIndex++;
              updateAndLog(improvementIndex, mol, molCopyR, molState, newMolState, overallFit, newOverallFit, logger, structureIndex, fitStep, ed, params);
        ...
        for (int secondarySectionIndex = 0; secondarySectionIndex < mol[structureIndex].getSubsecSize(chainNumber) - 1; secondarySectionIndex++) {
          ...
          if ((doAll == true) || (std::find(vary_sec_list_list[structureIndex].begin(), vary_sec_list_list[structureIndex].end(), totalIndex) != vary_sec_list_list[structureIndex].end())) {
            ...
            ktlMolecule newMol = mol[structureIndex];
            bool cacaDist = modifyMolecule(newMol, mol[structureIndex], indexCh, chainNumber);
            ...
            if (cacaDist == false) {
              ...
              if (checkTransition(newOverallFit.first, overallFit.first, uProb, fitStep, params.noScatterFitSteps)) {
                ...
                improvementIndex++;
                updateAndLog(improvementIndex, mol, newMol, molState, newmolState, overallFit, newOverallFit, logger, structureIndex, fitStep, ed, params);
                logger.consoleChange("fitImprove", params);
    ...
    molStateSet[historicFitIndex].updateMolecule(mol);
    sortVec(molStateSet);
