...
#
def get_residue_map_with_unknown(code_type=1):
    """
    Creates a mapping function that returns 'UNK' for non-standard residues.
    """
    standard_map = get_residue_map()

    if code_type == 1:

        def map_residue(res):
            return standard_map.get(res, 'U')

    elif code_type == 3:

        def map_residue(res):
            return standard_map.get(res, 'UNK')

    else:
        raise ValueError('Sequence code needs to be 1 or 3, you have:', code_type)
...
def pull_structure_from_pdb(pdb_file):
    ...
    if len(chains) > 1:
        ...
        for c in chains:
            ...
            sequence_chains.append( M.data['resname'][cond].map( get_residue_map_with_unknown() ).values )
        ...
        sequence_chains = [M.data['resname'][ca_cond].map( get_residue_map_with_unknown() ).values]
...
#         !./generate_structure {fingerprint_file} {coords_file} {outputname} {l}
        result = subprocess.run(['build/bin/generate_structure', fingerprint_file, coords_file, outputname, str(l)], capture_output=True, text=True)
